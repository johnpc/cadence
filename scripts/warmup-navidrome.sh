#!/usr/bin/env bash
# Prime the Navidrome auth path so the cold-start latency (cloudflared tunnel +
...
if [ -z "${URL:-}" ]; then
  echo "warmup: no VITE_NAVIDROME_URL set — skipping"
  exit 0
fi

BODY=$(printf '{"username":"%s","password":"%s"}' "${TEST_USERNAME:-}" "${TEST_PASSWORD:-}")
...
for attempt in 1 2 3 4 5; do
  ...
  if [ "$code" = "200" ]; then
    echo "warmup: Navidrome auth is warm"
    exit 0
  fi
  sleep 3
done
