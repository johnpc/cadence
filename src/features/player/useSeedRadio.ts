...
import { getAlbumTracks, getSimilarSongs } from '../../lib/navidromeItems';
 ...
 * queue with similar songs in the background. getSimilarSongs2 is slow
...
export function useSeedRadio() {
  ...
  return useCallback(
    async (seedId: string) => {
      const tracks = await getAlbumTracks(seedId).catch(() => []);
      if (tracks.length) {
        playQueue(tracks, 0);
        // Extend with the radio mix behind the now-playing album tracks.
        void getSimilarSongs(seedId)
          .then((mix) => {
      ...
      const mix = await getSimilarSongs(seedId);
